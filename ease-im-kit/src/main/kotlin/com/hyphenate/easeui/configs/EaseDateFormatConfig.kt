 ...
 * Date format configs.
 */
class EaseDateFormatConfig {

    /**
     * Whether use the default locale for conversation and chat date format.
     */
    var useDefaultLocale: Boolean = false
        get() {
            if (field) return field
            if (EaseIM.isInited()) {
                return EaseIM.getContext()?.resources?.getBoolean(R.bool.ease_date_use_default_locale) ?: false
            }
            return false
        }
    ...
    var convTodayFormat: String = ""
        get() {
            ...
            if (EaseIM.isInited()) {
                EaseIM.getContext()?.resources?.getStringArray(R.array.ease_conversation_date_format_array)?.let {
    ...
    var chatTodayFormat: String = ""
        get() {
            if (field.isNotEmpty()) return field
            if (EaseIM.isInited()) {
                EaseIM.getContext()?.resources?.getStringArray(R.array.ease_chat_date_format_array)?.let {
