 ...
 */
suspend fun ChatGroupManager.fetchJoinedGroupsFromServer(page:Int):MutableList<ChatGroup>{
    return suspendCoroutine{ continuation ->
        asyncGetJoinedGroupsFromServer(page,20,false,false,ValueCallbackImpl(
            onSuccess = {
                continuation.resume(it)
            },
            onError = {code,message-> continuation.resumeWithException(ChatException(code, message)) }
