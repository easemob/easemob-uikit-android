...
class EaseAtMessageHelper private constructor() {
    ...
    private val atMeGroupList: MutableSet<String> by lazy {
        EasePreferenceManager.getInstance().atMeGroups?.toMutableSet() ?: mutableSetOf()
    }
    ...
    fun containsAtUsername(content: String?): Boolean {
        ...
        synchronized(toAtUserList) {
            for (username in toAtUserList) {
                var nick = EaseIM.getCache().getMessageUserInfo(username)?.name
    ...
    fun containsAtAll(content: String?): Boolean {
        val atAll = "@${EaseIM.getContext()?.getString(R.string.ease_all_members)}"
            .uppercase()
                ...
                it.iterator().next()?.run { 
                    if (isGroupChat()) {
                        ...
                        if (atMeGroupList.size != size) {
                            EasePreferenceManager.getInstance().atMeGroups = atMeGroupList
                        }
                    }
                }
            }
