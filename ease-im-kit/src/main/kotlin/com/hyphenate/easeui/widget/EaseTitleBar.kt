...
import com.hyphenate.easeui.databinding.EaseWidgetTitleBarBinding
...
class EaseTitleBar @JvmOverloads constructor(
    ...
    private val binding: EaseWidgetTitleBarBinding by lazy {
        EaseWidgetTitleBarBinding.inflate(LayoutInflater.from(context), this, true)
    }
    ...
    private fun parseStyle(context: Context, attrs: AttributeSet?, defStyle: Int) {
        context.obtainStyledAttributes(attrs, R.styleable.EaseTitleBar, defStyle, 0)?.let { a ->
            val toolBarDisplayHomeAsUpEnabled = a.getBoolean(R.styleable.EaseTitleBar_titleBarDisplayHomeAsUpEnabled, true)
            setDisplayHomeAsUpEnabled(toolBarDisplayHomeAsUpEnabled, a.getBoolean(R.styleable.EaseTitleBar_titleBarReplaceActionBar, false))
            a.getDimensionPixelSize(R.styleable.EaseTitleBar_titleBarLogoSize, -1).let {
                if (it != -1) {
                    setLogoSize(it)
                }
            }
            a.getDrawable(R.styleable.EaseTitleBar_titleBarLogo)?.let {
                setLogo(it)
            }
            if (a.hasValue(R.styleable.EaseTitleBar_titleBarMenuTitleColor)) {
                setMenuTitleColor(a.getColor(R.styleable.EaseTitleBar_titleBarMenuTitleColor, ContextCompat.getColor(context, R.color.ease_title_bar_menu_text_color)))
            }
            if (a.hasValue(R.styleable.EaseTitleBar_titleBarMenuIconTint)) {
                setMenuIconTint(a.getColor(R.styleable.EaseTitleBar_titleBarMenuIconTint, ContextCompat.getColor(context, R.color.ease_title_bar_menu_icon_tint_color)))
            }
            setTitleTextAppearance(context, a.getResourceId(R.styleable.EaseTitleBar_titleBarTitleTextAppearance, 0))
            setSubtitleTextAppearance(context, a.getResourceId(R.styleable.EaseTitleBar_titleBarSubtitleTextAppearance, 0))

            val title = a.getText(R.styleable.EaseTitleBar_titleBarTitle)
            if (!TextUtils.isEmpty(title)) {
                setTitle(title)
            }

            val subtitle = a.getText(R.styleable.EaseTitleBar_titleBarSubtitle)
            if (!TextUtils.isEmpty(subtitle)) {
                setSubtitle(subtitle)
            }

            setTitleCentered(a.getBoolean(R.styleable.EaseTitleBar_titleBarTitleCenter, false))

            toolbar.popupTheme = a.getResourceId(R.styleable.EaseTitleBar_titleBarPopupTheme, 0)

            val navIcon = a.getDrawable(R.styleable.EaseTitleBar_titleBarNavigationIcon)
            navIcon?.let {
                setNavigationIcon(it)
            }

            if (a.hasValue(R.styleable.EaseTitleBar_titleBarNavigationIconTint)) {
                setNavigationIconTint(a.getColor(R.styleable.EaseTitleBar_titleBarNavigationIconTint, -1))
            }

            val navDesc = a.getText(R.styleable.EaseTitleBar_titleBarNavigationContentDescription)
            if (!TextUtils.isEmpty(navDesc)) {
                toolbar.navigationContentDescription = navDesc
            }

            val logo = a.getDrawable(R.styleable.EaseTitleBar_titleBarLogo)
            logo?.let { setLogo(it) }

            val logoDesc = a.getText(R.styleable.EaseTitleBar_titleBarLogoDescription)
            if (!TextUtils.isEmpty(logoDesc)) {
                setLogoDescription(logoDesc)
            }

            if (a.hasValue(R.styleable.EaseTitleBar_titleBarTitleTextColor)) {
                setTitleTextColor(a.getColorStateList(R.styleable.EaseTitleBar_titleBarTitleTextColor))
            }

            if (a.hasValue(R.styleable.EaseTitleBar_titleBarSubtitleTextColor)) {
                setSubtitleTextColor(a.getColorStateList(R.styleable.EaseTitleBar_titleBarSubtitleTextColor))
            }

            if (a.hasValue(R.styleable.EaseTitleBar_titleBarMenu)) {
                inflateMenu(a.getResourceId(R.styleable.EaseTitleBar_titleBarMenu, 0))
            }
    ...
    fun setDisplayHomeAsUpEnabled(enableDisplayHomeAsUp: Boolean, replaceActionBar: Boolean = false) {
        if (replaceActionBar) {
            ...
            if (enableDisplayHomeAsUp) {
                toolbar.setNavigationIcon(R.drawable.ease_default_navigation_icon)
            } else {
    ...
    fun getLogoView(): EaseImageView? {
        return binding.ivIcon
    }

    fun getStatusView(): TextView {
        return binding.tvStatus
    }
