...
open class EaseChatRowFile @JvmOverloads constructor(
    ...
    protected fun downloadAttachment(isThumbnail: Boolean) {
        message?.let {
            ...
            if (isThumbnail) {
                ChatClient.getInstance().chatManager().downloadThumbnail(it.getMessage())
            } else {
                ChatClient.getInstance().chatManager().downloadAttachment(it.getMessage())
            }
        }
    }
    ...
    protected fun setMessageDownloadCallback(isThumbnail: Boolean) {
        message?.run {
            ...
            if (canDownload) {
                setDownloadStatusCallback(CallbackImpl(
                    onSuccess = {
