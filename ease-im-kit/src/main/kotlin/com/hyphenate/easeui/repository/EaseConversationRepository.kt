...
class EaseConversationRepository(
    ...
    suspend fun loadData(): List<EaseConversation> =
        withContext(Dispatchers.IO) {
            ...
            if (hasLoaded) {
                ...
                chatManager.allConversationsBySort?.map { it.parse() } ?: listOf()
            }
        }
    ...
    suspend fun fetchConvInfo(conversationList: List<EaseConversation>) =
        withContext(Dispatchers.IO) {
            val map = conversationList
                .filter {
                    EaseIM.getCache().getConvInfo(it.conversationId) == null
                }
                .groupBy {
                    it.conversationType
                }.mapValues { (_, conv)->
                    conv.map { it.conversationId }
                }
            EaseIM.getConversationInfoProvider()?.fetchProfilesBySuspend(map)
        }
}