...
class EaseImageViewHolder(itemView: View) : EaseChatRowViewHolder(itemView) {
    override fun onBubbleClick(message: EaseMessage?) {
        ...
        val imgBody: ChatImageMessageBody? = message?.getMessage()?.body as? ChatImageMessageBody
        ...
        if (EaseFileUtils.isFileExistByUri(mContext, imgUri)) {
            ...
            // first
            EaseShowBigImageActivity.actionStart(mContext, message?.getMessage()?.msgId, imgBody?.fileName)
        }
    }
