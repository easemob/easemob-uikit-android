...
class EaseChatLayout @JvmOverloads constructor(
    ...
    ChatMessageListener, ChatConversationListener {
    ...
    private val chatBinding: EaseLayoutChatBinding by lazy { EaseLayoutChatBinding.inflate(
                ...
                item?.let {
                    when(it.menuId) {
                        ...
                        R.id.action_chat_delete -> {
                            chatDialogController.showDeleteDialog(mContext) {
    ...
    override fun onDetachedFromWindow() {
        ...
        EaseChatCacheDataController.clear(conversationId)
    }
    ...
    fun init(conversationId: String?, chatType: EaseChatType?, loadDataType: EaseLoadDataType = EaseLoadDataType.LOCAL) {
        ...
        viewModel?.setupWithToUser(conversationId, chatType, loadDataType == EaseLoadDataType.THREAD)
    ...
    fun inputAtUsername(username: String?, autoAddAtSymbol: Boolean) {
        ...
        username?.let {
            ...
            val user: EaseUser? = EaseIM.getCache().getMessageUserInfo(it)?.toUser() ?: EaseIM.getUserProvider()?.getUser(it)?.toUser()
            atUser = user?.getNickname()
            atUser = atUser?:it
    ...
    override fun resendMessage(message: ChatMessage?) {
        viewModel?.resendMessage(message?.get())
    }
    ...
    override fun recallMessage(message: ChatMessage?) {
        viewModel?.recallMessage(message?.get())
    }
    ...
    override fun deleteLocalMessageSuccess(message: EaseMessage?) {
        chatBinding.layoutChatMessage.removeMessage(message?.getMessage())
    }

    override fun deleteLocalMessagesSuccess() {
        chatBinding.layoutChatMessage.refreshMessages()
    }

    override fun recallMessageFinish(originalMessage: EaseMessage?, notification: EaseMessage?) {
        recallMessageListener?.recallSuccess(originalMessage?.getMessage(), notification?.getMessage())
        chatBinding.layoutChatMessage.refreshMessages()
    }
    ...
    override fun onSendMessageSuccess(message: EaseMessage?) {
        chatBinding.layoutChatMessage.refreshToLatest()
        listener?.onSuccess(message?.getMessage())
    }

    override fun onSendMessageError(message: EaseMessage?, code: Int, error: String?) {
        refresh(message)
        listener?.onError(code, error)
    }

    override fun onSendMessageInProgress(message: EaseMessage?, progress: Int) {

    }

    override fun sendMessageFinish(message: EaseMessage?) {
        chatMessageReplyController.clearReplyMessageExt(message?.getMessage())
        chatBinding.layoutChatMessage.refreshToLatest()
    }

    override fun addReactionMessageSuccess(message: EaseMessage?) {
        refresh(message)
        reactionMessageListener?.addReactionMessageSuccess(message?.getMessage())
    }

    override fun addReactionMessageFail(message: EaseMessage?, code: Int, error: String?) {
        reactionMessageListener?.addReactionMessageFail(message?.getMessage(), code, error)
    }

    override fun removeReactionMessageSuccess(message: EaseMessage?) {
        refresh(message)
        reactionMessageListener?.removeReactionMessageSuccess(message?.getMessage())
    }

    override fun removeReactionMessageFail(message: EaseMessage?, code: Int, error: String?) {
        reactionMessageListener?.removeReactionMessageFail(message?.getMessage(), code, error)
    }

    override fun onModifyMessageSuccess(messageModified: EaseMessage?) {
        refresh(messageModified)
        modifyMessageListener?.onModifyMessageSuccess(messageModified?.getMessage())
    }
    ...
    override fun reportMessageSuccess(msgId: String) {
        reportMessageListener?.onReportMessageSuccess(msgId)
    }

    override fun reportMessageFail(msgId: String, code: Int, error: String) {
        ...
        if (messageReactionChangeList != null) {
            for (reactionChange in messageReactionChangeList) {
                if (conversationId == reactionChange.conversionID) {
                    chatBinding.layoutChatMessage.refreshMessage(
                        ChatClient.getInstance().chatManager()
                            .getMessage(reactionChange.messageId)
                    )
                }
            }
        }
    ...
    private fun sendReadAck(message: ChatMessage) {
        if (EaseIM.getConfig()?.chatConfig?.enableSendChannelAck == true) {
    ...
    private fun refresh(message: EaseMessage?) {
        chatBinding.layoutChatMessage.refreshMessage(message?.getMessage())
    }
