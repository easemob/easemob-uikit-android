    ...
    override fun setData(item: EaseConversation?, position: Int) {
        item?.let {
            with(viewBinding) {
                ...
                EaseIM.getConversationInfoProvider()?.getSyncProfile(item.conversationId, item.conversationType)?.let { profile ->
                ...
                item.lastMessage?.let {
                    if (it.chatType != ChatType.Chat) {
                        if (it.isAlertMessage()) {
                            ...
                            val name = it.getUserInfo()?.name ?: it.from
                            message.text = name.plus(": ")
                            message.append(it.getMessageDigest(itemView.context)
                                .getEmojiText(itemView.context, message.getTextHeight()))
                        }
                    } else {
                        message.text = it.getMessageDigest(itemView.context)
