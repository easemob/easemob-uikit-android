            ...
            override fun onUserListItemClick(v: View?, position: Int, user: EaseUser?) {
                ...
                var showName = when (chatType) {
                    ...
                    EaseChatType.CHATROOM -> {
                        ChatClient.getInstance().chatroomManager().getChatRoom(conversationId)?.let { chatroom ->
                            chatroom.name
                        } ?: conversationId
                    }
                    else -> {
                        ChatClient.getInstance().chatManager().getConversation(conversationId)?.let { conv ->
                            conv.latestMessageFromOthers?.getUserInfo()?.name
                        } ?: conversationId
                    }
                }
                EaseIM.getConversationInfoProvider()?.getSyncProfile(conversationId, chatType.getConversationType())?.let { profile ->
