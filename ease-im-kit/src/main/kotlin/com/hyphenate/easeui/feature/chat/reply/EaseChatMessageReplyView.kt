...
class EaseChatMessageReplyView @JvmOverloads constructor(
    ...
    private fun initAttrs(context: Context, attrs: AttributeSet?) {
        ...
        context.obtainStyledAttributes(attrs, R.styleable.EaseChatMessageReplyView).let { a ->
            isSender = a.getBoolean(R.styleable.EaseChatMessageReplyView_ease_chat_message_reply_is_sender, false)
            isHistory = a.getBoolean(R.styleable.EaseChatMessageReplyView_ease_chat_message_reply_is_history, false)
            a.recycle()
        }
        if (!isHistory) {
            if (isSender) {
    ...
    fun updateMessageInfo(message: ChatMessage?): Boolean {
        ...
        val jsonObject: JSONObject? = try {
            val msgQuote = message.getStringAttribute(EaseConstant.QUOTE_MSG_QUOTE, null)
            if (msgQuote.isNullOrEmpty()) {
                message.getJSONObjectAttribute(EaseConstant.QUOTE_MSG_QUOTE)
            } else {
                JSONObject(msgQuote)
            }
    ...
    private fun textTypeDisplay(quoteContent: String) {
        if (this.quoteMessage?.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_BIG_EXPRESSION, false) == true) {
            ...
            this.quoteContent.text = quoteContent.getEmojiText(context, this.quoteContent.getTextHeight())
    ...
    private fun locationTypeDisplay(quoteContent: String) {
        if (this.quoteMessage == null) {
            ...
            (this.quoteMessage?.body as? ChatLocationMessageBody)?.let {
                this.quoteContent.text = resources.getString(R.string.ease_message_reply_location_content, it.address)
            }
        }
    }
    ...
    private fun voiceTypeDisplay(quoteContent: String) {
        if (this.quoteMessage == null) {
            ...
            (this.quoteMessage?.body as? ChatVoiceMessageBody)?.let {
                this.quoteContent.text = resources.getString(R.string.ease_message_reply_voice_content, it.length)
            }
        }
    }
    ...
    private fun fileTypeDisplay(quoteContent: String) {
        if (this.quoteMessage == null) {
            ...
            (this.quoteMessage?.body as? ChatFileMessageBody)?.let {
                this.quoteContent.text = resources.getString(R.string.ease_message_reply_file_content, it.fileName)
            }
        }
    }
