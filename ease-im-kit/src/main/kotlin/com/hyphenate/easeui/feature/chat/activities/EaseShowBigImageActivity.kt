 ...
 *
 */
class EaseShowBigImageActivity : EaseBaseActivity<EaseActivityShowBigImageBinding>() {
    private var default_res: Int = R.drawable.ease_default_image
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        default_res = intent.getIntExtra("default_image", R.drawable.ease_default_image)
        ...
        val emojiIconId: String? =
            intent.extras?.getString(EaseConstant.MESSAGE_ATTR_EXPRESSION_ID)
        ...
        if (EaseFileUtils.isFileExistByUri(this, uri)) {
            ...
            if (EaseFileUtils.isFileExistByUri(this, body.localUri)) {
    ...
    override fun getViewBinding(inflater: LayoutInflater): EaseActivityShowBigImageBinding? {
    ...
    private fun showBigExpression(emojiIconId: String?) {
        EaseIM.getEmojiconInfoProvider()?.getEmojiconInfo(emojiIconId)?.let {
            if (it.bigIcon != -1) {
                ...
                binding.image.setImageResource(R.drawable.ease_default_expression)
            }
        }
    }
    ...
    private fun downloadImage(msg: ChatMessage?) {
        ...
        val str1: String = resources.getString(R.string.ease_download_the_pictures)
        val pd = ProgressDialog(this)
        pd.setProgressStyle(ProgressDialog.STYLE_SPINNER)
        pd.setCanceledOnTouchOutside(false)
        pd.setMessage(str1)
        pd.show()
        msg.get().setDownloadStatusCallback(CallbackImpl(
            onSuccess = {
            ...
            onProgress = { progress ->
                if (progress > mProgress) {
                    mProgress = progress
                    mContext.mainScope().launch {
                        val str2: String = resources.getString(R.string.ease_download_the_pictures_new)
    ...
    override fun onDestroy() {
        ...
        if (isNeedDownload && !msgId.isNullOrEmpty()) {
            EaseFlowBus.with<EaseEvent>(EaseEvent.EVENT.UPDATE.name).post(lifecycleScope,
                EaseEvent(EaseEvent.EVENT.UPDATE.name, EaseEvent.TYPE.MESSAGE, msgId)
            )
        }
    }
