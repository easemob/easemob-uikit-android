...
class EaseChatInputMenu @JvmOverloads constructor(
...
): FrameLayout(context, attrs, defStyleAttr), IChatInputMenu, EaseChatPrimaryMenuListener,
    EaseChatExtendMenuItemClickListener, EaseEmojiconMenuListener {

    private val TAG = EaseChatInputMenu::class.java.simpleName

    private val binding: EaseWidgetChatInputMenuContainerBinding by lazy { EaseWidgetChatInputMenuContainerBinding.inflate(
    ...
    init {
        showPrimaryMenu()
        if (extendMenu == null) {
            extendMenu = EaseChatExtendMenu(getContext())
    ...
    private fun showPrimaryMenu() {
        if (primaryMenu == null) {
            primaryMenu = EaseChatPrimaryMenu(getContext())
        }
        primaryMenu?.let {
            if (it is View) {
